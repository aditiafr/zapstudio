...
<div class="flex items-center justify-between">
    <h1 class="text-3xl text-black pb-6">All Paket</h1>
...
<div class="w-full mt-12">
    ...
    <div class="w-full flex justify-end mb-4">
        <form action="{{ route('paket.index') }}" method="GET" class="w-64">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($paket as $index => $pack)
                <tr class="hover:bg-grey-lighter">
                    <td class="py-4 px-6 border-b border-grey-light">{{ $paket->firstItem() + $index }}</td>
                    ...
                    <td class="py-4 px-6 border-b border-grey-light">
                        <div class="flex gap-3 justify-center">
                            ...
                            <button data-modal-target="popup-modal-{{ $pack->id_paket }}" data-modal-toggle="popup-modal-{{ $pack->id_paket }}" class="bg-red-500 hover:bg-red-700 text-white py-2 px-4 rounded" type="button">
                                Delete
                            </button>
                        </div>
                    </td>
                ...
                <div id="popup-modal-{{ $pack->id_paket }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="p-4 md:p-5 text-center">
                                <form action="{{ route('paket.destroy', $pack->id_paket) }}" method="POST">
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this paket? with package name {{$pack->namapaket}}</h3>
                                    <button data-modal-hide="popup-modal-{{ $pack->id_paket }}" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                        Yes, I'm sure
                                    </button>
                                    <button data-modal-hide="popup-modal-{{ $pack->id_paket }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No, cancel</button>
                                </form>
                            </div>
                        </div>
                    </div>
